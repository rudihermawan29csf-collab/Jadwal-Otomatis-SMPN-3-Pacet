...
import { WalasEntry, Teacher, WalasDocument, SchoolConfig, CLASSES, DutyDocument } from '../types';
...
export const WalasLetter: React.FC<Props> = ({ documents, setDocuments, teachers, schoolConfig, dutyDocuments, activeDutyDocId, onSwitchDutyDoc }) => {
    ...
    const getAvailableClasses = (currentId: string) => {
        const usedByOthers = activeDoc.entries.filter(e => e.id !== currentId).map(e => e.className);
        ...
        <div className="flex flex-col items-center">
            ...
            <div className="w-full max-w-4xl bg-blue-50 p-4 rounded-t border-x border-t border-blue-200 no-print flex flex-wrap gap-2 justify-between items-center">
                ...
                <div className="flex gap-2">
                    <button onClick={handleCreateNewDoc} className="bg-green-600 text-white px-3 py-1 rounded text-xs font-bold shadow-sm transition">+ Baru</button>
                    <button onClick={handleDuplicateDoc} className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold transition">Duplikat</button>
                    <button onClick={handleDeleteDoc} className="bg-red-600 text-white px-3 py-1 rounded text-xs font-bold transition">Hapus</button>
                </div>
            </div>

            {/* Config Panel */}
            <div className="w-full max-w-4xl bg-gray-100 p-4 rounded-b mb-4 border border-gray-300 no-print flex flex-wrap gap-4 items-end shadow-inner">
                <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-bold uppercase text-gray-500">Nomor SK (Tengah)</label>
                    <input type="text" className="border p-1 rounded text-sm w-24 text-center font-mono" value={activeDoc.skNumberCode} onChange={e => updateActiveDoc({ skNumberCode: e.target.value })} />
                </div>
                <div className="flex flex-col gap-1">
            ...
            {/* Document Preview */}
            <div className="w-full max-w-4xl bg-white shadow-2xl p-10 mb-8 font-sans border border-gray-200 print:shadow-none print:p-0">
                <div className="flex justify-end mb-10 text-[8px] leading-tight">
                    <table className="w-64 border-none">
                        <tbody>
                            <tr className="border-none"><td className="w-20 p-0 border-none">Lampiran 5.</td><td className="p-0 border-none">: Keputusan Kepala SMPN 3 Pacet</td></tr>
                            ...
                            editingId === row.id && editForm ? (
                                <tr key={row.id} className="bg-blue-50">
                                    ...
                                    <td className="border border-black p-2">
                                        <select className="w-full border mb-1 text-[10px]" onChange={handleTeacherSelect} defaultValue="">
                                    ...
                                    <td className="border border-black p-2 no-print">
                                        <button onClick={handleSave} className="bg-green-600 text-white w-full text-[9px] mb-1 py-1 rounded">Simpan</button>
                                ...
                                <tr key={row.id}>
                                    ...
                                    <td className="border border-black p-2 text-center align-top font-bold">Wali Kelas {row.className}</td>
                                    <td className="border border-black p-2 text-center align-top no-print">
                                        <button onClick={() => handleEdit(row)} className="text-blue-600 hover:underline">Edit</button><br/>
                ...
                <button onClick={handleAdd} className="w-full mt-3 border-2 border-dashed border-blue-200 text-blue-500 p-2 no-print hover:bg-blue-50 transition-all font-bold rounded">+ Tambah Baris Wali Kelas</button>
            ...
            <div className="no-print flex gap-4 mb-12">
                <button onClick={() => exportWalasPDF(activeDoc.entries, 'A4', fullSkNumber, formattedDate, activeDoc.semester, activeDoc.academicYear, schoolConfig)} className="bg-red-600 text-white px-6 py-2 rounded font-bold shadow hover:bg-red-700">PDF A4</button>
                <button onClick={() => exportWalasPDF(activeDoc.entries, 'F4', fullSkNumber, formattedDate, activeDoc.semester, activeDoc.academicYear, schoolConfig)} className="bg-blue-600 text-white px-6 py-2 rounded font-bold shadow hover:bg-blue-700">PDF F4</button>
                <button onClick={() => exportWalasWord(activeDoc.entries, fullSkNumber, formattedDate, activeDoc.semester, activeDoc.academicYear, schoolConfig)} className="bg-indigo-600 text-white px-6 py-2 rounded font-bold shadow hover:bg-indigo-700 flex items-center gap-2">
